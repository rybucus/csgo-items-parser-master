...
namespace CsgoItemsParser.Parser
{
    public class ItemsParser
    {
        ...
        public ItemsParser()
        {
            ...
            _itemsFileContent = File.ReadAllText(Path.Combine(CounterStrikePath, "game", "csgo", "scripts", "items", "items_game.txt"));
            _translationFileContent = File.ReadAllText(Path.Combine(CounterStrikePath, "game", "csgo", "resource", "csgo_english.txt"));

            _itemsRoot = VdfParser.Parse(_itemsFileContent);
        }
        ...
        {
            // The old regex only matched entries where "prefab" was the field immediately
            // following "name" and where the previous entry's closing brace sat right before
            // the index - both assumptions break for large parts of items_game.txt (the very
            // first entry, loadout-slot placeholders, anything with a different field order),
            // silently dropping the overwhelming majority of real item definitions. Walking the
            // actual parsed key/value tree for "items_game" -> "items" instead picks up every
            // direct child regardless of its internal layout.
            var itemsGame = _itemsRoot.FindChild("items_game");
            var items = itemsGame?.FindChild("items");

            if (items == null || items.Children == null) return null;
            if (items.Children.Count == 0) return null;

            List<ItemDefinition> itemDefinitions = new List<ItemDefinition>(items.Children.Count);

            foreach (var item in items.Children)
            {
                if (!item.IsBlock) continue;
                if (!int.TryParse(item.Key, out int index)) continue;

                var nameNode = item.FindChild("name");
                if (nameNode == null || nameNode.IsBlock || string.IsNullOrEmpty(nameNode.Value)) continue;
